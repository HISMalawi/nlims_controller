...
RSpec.describe 'Tests API', type: :request do
  ...
  path '/api/v2/tests/{tracking_number}/add_test_to_order' do
    post 'Add a test to an existing order' do
      tags 'Tests'
      consumes 'application/json'
      produces 'application/json'
      security [tokenAuth: []]
      description 'Add a test to an existing order'
      parameter name: :tracking_number, in: :path, type: :string, required: true
      parameter name: :test_payload, in: :body, schema: {
        type: :object,
        required: %w[test_status test_type],
      ...
      response '200', 'test added successfully' do
        schema type: :object,
               properties: {
                 message: { type: :string, example: 'test added successfully' },
                 data: { type: :object }
               }
        run_test!
      end
      ...
      response '422', 'Error: Unprocessable Entity' do
        schema type: :object,
               properties: {
                 error: { type: :boolean, example: true },
                 message: { type: :string, example: 'test type not found' },
                 data: { type: :object, example: {} }
               }
        run_test!
      end
